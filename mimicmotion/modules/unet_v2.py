...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def preprocess(self, timestep, encoder_hidden_states, added_time_ids, num_frames, device):
        ...
        if not torch.is_tensor(timesteps):
            ...
            timesteps = torch.tensor([timesteps], dtype=torch.float16, device=device)
        ...
        # emb: [batch, channels] -> [batch * frames, channel
        # print(emb, emb.device, emb.shape, emb.dtype)
        # print(num_frames)
        if isinstance(num_frames, torch.Tensor):
            num_frames = num_frames.to("cuda:0")
    ...
    def forward(
        ...
        image_only_indicator = torch.zeros(1, 16, dtype=sample.dtype, device=sample.device)
        ...
        sample = sample.reshape(1, 16, *sample.shape[1:])
